...
updateRequestNodeHandler :: Auth.Token -> Int -> Id Request -> UpdateRequestNode -> ClientM ()
deleteRequestNodeHandler :: Auth.Token -> Int -> Id Request -> ClientM ()
    ...
    describe "update request node" $ do
      ...
      it "returns 404 if the request node doesnt belong to the account" $ \clientEnv ->
        cleanDBAndCreateAccount $ \Test { connection, token } -> do
          ...
          let nodeId = coerce $ head requestNodes & _requestNodeId
