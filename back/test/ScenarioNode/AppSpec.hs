...
{-
      it "modifies a scenario folder" $ \clientEnv ->
        cleanDBAfter $ \connection -> do
          accountId <- insertFakeAccount defaultNewFakeAccount1 connection
          ScenarioCollection requestCollectionId requestNodes <- insertSampleScenarioCollection accountId connection
          let nodeId = head requestNodes ^. requestNodeId
          token <- signedUserToken accountId
          _ <- try clientEnv (updateScenarioNodeHandler token requestCollectionId nodeId updateScenarioNode)
